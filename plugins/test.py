...
async def start_clone_bot(FwdBot, data=None):
   ...
   async def iter_messages(
      ...
      continuous: bool = False
        ...
        """Iterate through a chat sequentially without skipping messages."""
        current = offset
        fetched_count = 0
        consecutive_empty = 0
        MAX_EMPTY_BATCHES = 5
        ...
        while True:
            ...
            id_range = list(range(current, current + batch))

            try:
                messages = await self.get_messages(chat_id, id_range)
            ...
            for message in valid_messages:
                yield message
                fetched_count += 1
                if not continuous and limit > 0 and fetched_count >= limit:
                    return
